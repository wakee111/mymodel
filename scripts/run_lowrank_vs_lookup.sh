#!/bin/bash
# ETTh1 + ETTh2: RecurrentCycle (lookup) baseline vs LowRankCycle (lowrank)
# GPU 0 = lookup, GPU 1 = lowrank (rank=4)
...
for PRED in 96 192 336 720; do
    ...
    # ===== GPU 0: Baseline (lookup) =====
    echo "=== ETTh1 lookup pred=$PRED ==="
    ...
    # ===== GPU 1: LowRank (rank=4) =====
    echo "=== ETTh1 lowrank pred=$PRED ==="
