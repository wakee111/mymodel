#!/usr/bin/env python
"""
Multi-stage training for Solar: MRT warmstart + Freq fine-tune + FreDF joint optimize.
...
Stage 3: Unfreeze all, joint fine-tune with lower LR + FreDF(alpha=0.85).

Usage:
    CUDA_VISIBLE_DEVICES=0 /data/data_linsf/Anaconda3/envs/TimeSeries_huaji/bin/python \
        scripts/multi_stage_train_solar.py --pred_len 720 [--stage1_only]
"""
...
def build_args(pred_len, seed=2024):
    """Build args namespace for Solar dataset."""
    parser = argparse.ArgumentParser()
    # Required
    parser.add_argument('--is_training', type=int, default=1)
    parser.add_argument('--model_id', type=str, default='Solar_96_{}'.format(pred_len))
    parser.add_argument('--model', type=str, default='CycleNet')
    parser.add_argument('--data', type=str, default='Solar')
    # Data
    parser.add_argument('--root_path', type=str, default='./dataset/Solar/')
    ...
    # CycleNet
    parser.add_argument('--cycle', type=int, default=144)
...
def make_setting(args, stage, seed=2024):
    """Build setting string for checkpoint paths."""
...
def stage2_add_freq_and_freeze(args, device, stage1_ckpt):
    ...
    # Copy matching keys from stage1
    ...
    # Freeze everything except freq_blocks
